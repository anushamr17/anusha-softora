<?php
    ...
    $register_email = mysqli_real_escape_string($conn, $_POST["remail"]);
    $register_pwd = mysqli_real_escape_string($conn, $_POST["rpass"]);
    $register_repwd = mysqli_real_escape_string($conn, $_POST["repass"]);
    ...
    if (!preg_match($regex_email, $register_email) || !preg_match($regex_password, $register_pwd)) {
      ...
      {
        $sql = "INSERT INTO user (email, password) VALUES ('$register_email', '$register_pwd')";
        if (($conn->query($sql)))
        {
          $err_no=0;
